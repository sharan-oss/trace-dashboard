...
- [ ] **Slice B — Meta sync**: three tables, Meta API client, service identity, admin account mapping, first sync. Planned in full at `docs/superpowers/plans/2026-08-09-meta-ads-sync-implementation.md`; branch `slice-b-meta-ads-sync` holds the plan only. **Unblocked 2026-08-10** — Meta access proven live via `scripts/meta-access-probe.ts` (see Open questions); the plan needs refresh before execution: `sync_runs`→`ad_sync_runs` rename, new migration timestamp (`20260810100000` is taken), pin `v26.0` not `v25.0`, sync-first insights (async only for backfill), 28-day nightly re-fetch window (not 7), and account discovery via `me/assigned_ad_accounts` (system-user tokens cannot traverse `/{business_id}/owned_ad_accounts`).
