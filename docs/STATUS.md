...
- [ ] **Phase 1 — Revenue overview (admin)**: total revenue, per-client/per-product breakdown, conversion rate, `paid_at`-based time series. **UI not started**, but both prerequisites are now done — the visual design system, and the metrics foundation above. Build it on the views, not the raw tables.
